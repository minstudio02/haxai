...
const waitForHaxballApiInit = async (page, maxWaitTime = 10 * 1000) => {
  await page.evaluate(() => {
    const startTime = Date.now();

    return new Promise((resolve, reject) => {
      (function loop() {
        if (window.HBInit) return resolve();

        // retry if it's less than N miliseconds spent on retries
        if ((Date.now() - startTime) < maxWaitTime) return setTimeout(loop);

        // probably never possible, but this is better than endless loop
        reject(new Error('Hax API object does not appear'));
      }());
    });
  });
}
async function launchServer(args) {
    ...
    await waitForHaxballApiInit(page);
